...
class newModel:
    ...
    def __init__(self, X, Xval,
                 n_latent=50, n_hidden=100, n_samples=1, n_latent_tilda = 1,
        ...
        self.blocks = blocks
        self.alpha = alpha
        self.bankpdays = bankpdays
        self.bankall = bankall
        ...
        self.n_latent_tilda = n_latent_tilda
        ...
        with tf.variable_scope('encoder'):
            if self.encode_mask:
                self.q_mu, self.q_log_sig2 = self.encoder(tf.concat([self.in_pl, self.s_pl], axis = -1))
            else:
        ...
        #self.l_z_both = q_z.sample(self.n_pl)  # shape [n_samples, batch_size, d]
        self.l_z_both = tf.transpose(q_z.sample(self.n_pl), perm=[1, 0, 2])  # shape [batch_size, n_samples, d]

        self.l_z = self.l_z_both[:,:,:self.n_latent]
        self.l_z_tilda = self.l_z_both[:,:,self.n_latent:]
        
        # ---- parameters from decoder
        if out_dist in ['gauss', 'normal', 'truncated_normal']:

            with tf.variable_scope('data_process'):
                mu = self.gauss_decoder(self.l_z)
                if self.sig2 is None:
                    logstd = tf.get_variable('logstd', shape=[])
                else:
                    logstd = tf.log(tf.sqrt(self.sig2), name = 'logstd')

            # ---- p(x|z)
            if out_dist == 'truncated_normal':
                p_x_given_z = tfp.distributions.TruncatedNormal(loc=mu, scale=tf.exp(logstd), low=0.0, high=1.0)
            else:
        ...
        with tf.variable_scope('missing'):
            ...
            self.logits_miss = self.bernoulli_decoder_miss(tf.concat([self.l_out_mixed, self.l_z_tilda], axis = -1))

        with tf.variable_scope('missing', reuse=tf.AUTO_REUSE):
            self.logits_miss_gen = self.bernoulli_decoder_miss(tf.concat([self.l_out_sample, self.l_z_tilda], axis = -1))
        ...
        # ---- evaluate s in p(s|x)
        if self.bankpdays:
            self.log_p_s_given_x = tf.reduce_sum(self.p_s_given_x.log_prob(tf.expand_dims(tf.reshape(self.s_pl[:,2], [-1,1]), axis=1)), axis=-1)
        elif self.bankall:
        ...
        self.log_q_z_given_x = tf.reduce_sum(q_z2.log_prob(self.l_z_both), axis=-1)

        # ---- evaluate the z-samples in the prior
        prior = tfp.distributions.Normal(loc=0.0, scale=1.0)
        self.log_p_z = tf.reduce_sum(prior.log_prob(self.l_z_both), axis=-1)

        # ---- newModel:
        self.newModel = self.get_newModel(self.log_p_x_given_z,
                                          self.log_p_s_given_x,
                                          self.log_q_z_given_x,
                                          self.log_p_z, self.alpha)
        ...
        self.train_writer = tf.summary.FileWriter(name + '/tensorboard/newModel_train/{}/'.format(timestamp),
                                                  self.sess.graph)
        self.val_writer = tf.summary.FileWriter(name + '/tensorboard/newModel_val/{}/'.format(timestamp),
                                                self.sess.graph)
    ...
    def encoder(self, x):
        ...
        elif self.complexity == "med":
            
            x = keras.layers.Dense(units=self.n_hidden, activation=self.activation, name='l_enc1')(x)

        mu = keras.layers.Dense(units=self.n_latent+self.n_latent_tilda, activation=None, name='q_mu')(x)

        log_sig2 = keras.layers.Dense(units=self.n_latent+self.n_latent_tilda, activation=lambda x: tf.clip_by_value(x, -10, 10),
                                   name='q_log_sigma')(x)
    ...
    def gauss_decoder(self, z):
        ...
        if self.complexity == "high":
            ...
            z = keras.layers.Dense(units=self.n_hidden, activation=self.activation, name='l_dec_gauss1')(z)
        elif self.decoder_structure == "ma":
            z = keras.layers.Dense(units=20, activation=self.activation, name='l_dec_gauss1')(z)
            z = keras.layers.Dense(units=10, activation=self.activation, name='l_dec_gauss2')(z)
    ...
    def bernoulli_decoder_miss(self, z):
        ...
        if self.missing_process == 'linear':
            ...
            logits = keras.layers.Dense(units=self.d, activation=None, name='y')(z)
        elif self.missing_process == 'linear_nsc':# no self censoring
            x = z[:,:,:self.d]
            lz = z[:,:, self.d:]
            #tf.expand_dims(tf.zeros_like(l_out_mixed[:,:,d]),axis = 2),
        
            #r_h= keras.layers.Dense(units=n_hidden, activation=tf.nn.tanh, name='rh'+str(d))(tf.concat([x_hat,l_z], 2))
            # r_logits = keras.layers.Dense(units=1, activation=None, name='r'+str(d))(r_h)
        
            # p_s_given_x = tfp.distributions.Bernoulli(logits=r_logits)
            # p_s_given_x_list.append(p_s_given_x)
            #logits = tf.zeros_like(z)
            #logit_list = []
            if self.blocks is None:
                if self.bankpdays:
                    j = 2
                    x_hat = tf.concat([x[:,:,:j], x[:,:,(j+1):]],2)
                    logit = keras.layers.Dense(units=1, activation=None, name='r'+str(j))(tf.concat([x_hat, lz],2))
                    logits = logit
                else:
                ...
                for j in range(num_blocks):
                    if j == 0:
                        mask = tf.concat((tf.zeros_like(x[:,:,:self.blocks[0]]), tf.ones_like(x[:,:,self.blocks[0]:])),axis = 2)
                    elif j == num_blocks-1:
                        mask = tf.concat((tf.ones_like(x[:,:,:sum(self.blocks[0:num_blocks-1])]), tf.zeros_like(x[:,:,sum(self.blocks[0:num_blocks-1]):])),axis = 2)
                    else:
                        mask = tf.concat((tf.ones_like(x[:,:,:sum(self.blocks[0:j])]), tf.zeros_like(x[:,:,sum(self.blocks[0:j]):sum(self.blocks[0:(j+1)])]), tf.ones_like(x[:,:,sum(self.blocks[0:(j+1)]):])),axis = 2)
    
                    x_hat = mask*x
                    #logit = sharedDense(tf.concat([x_hat, lz],2))
                    logit = keras.layers.Dense(units=self.blocks[j], activation=None, name='r'+str(j))(tf.concat([x_hat, lz],2))
                    if j == 0:
                        logits = logit
                    else:
                        logits = tf.concat((logits, logit), axis = 2)
            #     logit_list.append(logit)
            # logits = tf.stack(logit_list, axis=2)
                #logits[:,:,j] = logit
            # logits = keras.layers.Dense(units=self.d, activation=None, name='y')(z)    

        elif self.missing_process == 'linear_nsc_nonlinear_zt':# no self censoring
            x = z[:,:,:self.d]
            lz = z[:,:, self.d:]
            h1 = keras.layers.Dense(units=self.n_hidden, activation=tf.nn.tanh, name='h')(lz)
            #h1 = keras.layers.Dense(units=self.d, activation=tf.nn.tanh, name='h2')(h1)
            for j in range(self.d):
                x_hat = tf.concat([x[:,:,:j], x[:,:,(j+1):]],2)
                #tf.concat([x_hat, lz],2)
                logit = keras.layers.Dense(units=1, activation=None, name='r'+str(j))(tf.concat([x_hat, h1],2))
                if j == 0:
                    logits = logit
                else:
                    logits = tf.concat((logits, logit), axis = 2)
                    
        elif self.missing_process == 'linear_nsc_nonlinear_zt_2h':# no self censoring
            x = z[:,:,:self.d]
            lz = z[:,:, self.d:]
            h1 = keras.layers.Dense(units=self.n_hidden, activation=tf.nn.tanh, name='h')(lz)
            h1 = keras.layers.Dense(units=self.d, activation=tf.nn.tanh, name='h2')(h1)
            ...
            for j in range(self.d):
                x_hat = tf.concat([x[:,:,:j], x[:,:,(j+1):]],2)
                logit = keras.layers.Dense(units=self.n_hidden, activation=tf.nn.tanh, name='h'+str(j))(tf.concat([x_hat, lz],2))
                logit = keras.layers.Dense(units=1, activation=None, name='r'+str(j))(logit)
                if j == 0:
                    logits = logit
                else:
                    logits = tf.concat((logits, logit), axis = 2)
            ...
            lz = z[:,:, self.d:]
            # logits = tf.zeros_like(z)
            sharedDense = keras.layers.Dense(units=self.n_hidden, activation=tf.nn.tanh, name='h')
            
            if self.blocks is None:

                if self.bankpdays:
                    j = 2
                    mask = tf.concat((tf.ones_like(x[:,:,:j]), tf.expand_dims(tf.zeros_like(x[:,:,j]), axis = 2), tf.ones_like(x[:,:,(j+1):])),axis = 2)
                    x_hat = mask*x
                    logit = sharedDense(tf.concat([x_hat, lz],2))
                    logit = keras.layers.Dense(units=1, activation=None, name='r'+str(j))(logit)
                    logits = logit
                    ...
                    for j in range(self.d):#?
                        ...
                        if j == 0:
                            mask = tf.concat((tf.expand_dims(tf.zeros_like(x[:,:,0]), axis = 2), tf.ones_like(x[:,:,1:])),axis = 2)
                        elif j == self.d-1:
                            mask = tf.concat((tf.ones_like(x[:,:,:(self.d-1)]), tf.expand_dims(tf.zeros_like(x[:,:,-1]), axis = 2)),axis = 2)
                        else:
                            mask = tf.concat((tf.ones_like(x[:,:,:j]), tf.expand_dims(tf.zeros_like(x[:,:,j]), axis = 2), tf.ones_like(x[:,:,(j+1):])),axis = 2)
                ...
                for j in range(num_blocks):
                    ...
                    x_hat = mask*x
                    logit = sharedDense(tf.concat([x_hat, lz],2))
                    logit = keras.layers.Dense(units=self.blocks[j], activation=None, name='r'+str(j))(logit)
                    if j == 0:
                        logits = logit
                    else:
                        logits = tf.concat((logits, logit), axis = 2)
    ...
    def get_ELBO(self, q_z, lpxz): #vae elbo?
    ...
    def get_newModel(self, lpxz, lpmz, lqzx, lpz, alpha):
        """" the not-MIWAE ELBO """

        # ---- importance weights
        l_w = lpxz + alpha*lpmz + lpz - lqzx#
    ...
    def generator(self, z_both):
        x_hat, s_hat = self.sess.run([self.l_out_sample, self.logits_miss_gen], feed_dict={self.l_z_both: z_both}) # shape [batch_size, n_samples, d]
        return x_hat, s_hat #, self.logits_miss
